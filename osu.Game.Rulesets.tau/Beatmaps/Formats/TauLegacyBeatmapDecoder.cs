...
{
    public class TauLegacyBeatmapDecoder : LegacyBeatmapDecoder
    {
        ...
        {
            AddDecoder<Beatmap>(@"tau file format v", m => new TauLegacyBeatmapDecoder(Parsing.ParseInt(m.Split('v').Last())));
            SetFallbackDecoder<Beatmap>(() => new TauLegacyBeatmapDecoder());
        }

        public TauLegacyBeatmapDecoder(int version = LATEST_VERSION)
            : base(version)
        {
        }
        ...
        {
            // TODO: create ConvertHitObjectParser for tau beats.
            //var obj = parser.Parse(line);
            //if (obj != null)
            //    beatmap.HitObjects.Add(obj);
        }

        private int getOffsetTime(int time) => time;

        private double getOffsetTime() => 0;
